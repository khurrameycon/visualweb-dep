...
class CustomAgent(Agent):
    ...
    def detect_stuck_state(self, model_output, previous_actions_history):
        """Detect if agent is stuck and suggest recovery"""
        if len(previous_actions_history) >= 3:
            # Check if last 3 actions are scroll/click in same area
            recent_actions = previous_actions_history[-3:]
            if all('scroll' in str(action).lower() or 'click' in str(action).lower() for action in recent_actions):
                return True
        return False
        ...
        try:
            ...
            try:
                ...
                if hasattr(self, '_action_history'):
                    if self.detect_stuck_state(model_output, self._action_history):
                        # Insert recovery action
                        recovery_action = {"action_name": "handle_stuck_state", "args": {}}
                        model_output.action.insert(0, recovery_action)
                else:
                    self._action_history = []
                
                # Store action history
                self._action_history.append(model_output.action)
            ...
            actions = [a.model_dump(exclude_unset=True) if hasattr(a, 'model_dump') else a for a in model_output.action] if model_output else []
